...
class Images(Dataset):
    ...
    def __getitem__(self, idx):
        ...
        image = torchvision.transforms.ToTensor()(Image.open(img_path).convert("RGB").resize((32, 32)))
...
parser.add_argument("-eps", type=float, default=5e-3, help="Target for downscaling loss (L2)")
...
for ref_im, ref_im_name in dataloader:
    ...
    for j, (HR, LR) in enumerate(model(ref_im, **kwargs)):
        # kwargs["batch_size"] -- 1
        # ref_im.size() -- torch.Size([1, 3, 32, 32])
